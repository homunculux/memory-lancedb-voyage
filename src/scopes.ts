...
export class MemoryScopeManager implements ScopeManager {
  ...
  addScopeDefinition(scope: string, definition: ScopeDefinition): void {
    if (!/^[a-zA-Z0-9._:-]+$/.test(scope) || scope.length > 100) {
  ...
  setAgentAccess(agentId: string, scopes: string[]): void {
    if (!agentId || typeof agentId !== "string") throw new Error("Invalid agent ID");
    for (const scope of scopes) {
  ...
  importConfig(config: Partial<ScopeConfig>): void {
    this.config = {
      ...
      definitions: { ...this.config.definitions, ...config.definitions },
      agentAccess: { ...this.config.agentAccess, ...config.agentAccess },
    };
    this.validateConfiguration();
  }
